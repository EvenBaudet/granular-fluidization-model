...
eta = 1.8e-5 #coefficient de viscosité dynamique de l'air
rho_air = 1.2e-3
phi = np.zeros(N)
...
ax_eta  = create_mini_slider(0.18, "Viscosité cinématiquedu Fluide (η)", accent_eta)
...
s_rho = Slider(ax_rho, '', 0.5, 200, valinit=1.225, valfmt='%1.2f kg.m-3', color=accent_rho)
s_eta  = Slider(ax_eta, '', 1e-6, 1e-4, valinit=1.5e-5, valfmt='%1.0e m2.s-1', color=accent_eta)

for s in [s_v, s_m, s_rho, s_eta]:
    s.valtext.set_color('white')
    s.valtext.set_fontsize(8)
    s.valtext.set_fontweight('bold')

def update_params(val):
    global V, m, rho_air, eta, Kg, Kp, gamma_g, gamma_p
    V, m, rho_air, eta = s_v.val, s_m.val, s_rho.val, s_eta.val
