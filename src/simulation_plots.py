# -*- coding: utf-8 -*-
"""
Created on Thu May  8 19:55:51 2025
...
def force_fluide_gradient_pression(i,Lg,X,Y):
    ...
    k = maj_k(phi[i])
    return rho_air*g + (nu/k)*V #loi de Darcy cf Les milieux granulaires: Entre fluide et solide
...
def maj():
    global Ax,Ay,Vx,Vy,X,Y,rp,t,M,V,dV,phi
...
def determination_vitesse_fluidisation():
    while t < T:
        maj()
    
    L_t = np.linspace(0,T,int(T/dt) + 1)
    
    #premier filtrage des hautes frequences peu sélectif
    tau = 2.5e-2
    #S : signal filtré de la fraction volumique en fonction du temps
    S = [L_phi[0]]
    for i in range(1,len(L_phi)):
        S.append(S[i-1] + (dt/tau)*(L_phi[i-1]-S[i-1]))
    
    #dérivée du signal
    d_phi = []
    for i in range(len(S)-1):
        d_phi.append((S[i+1]-S[i])/dt)
    
    #filtrage des hautes frequences
    tau = 5e-2
    d_phi_filtre = [d_phi[0]]
    for i in range(1,len(d_phi)):
        d_phi_filtre.append( d_phi_filtre[i-1] + (dt/tau)*(d_phi[i-1]-d_phi_filtre[i-1]))    
    
    #détermination de la valeur correspondant a la fluidisation
    i = 200
    while abs(d_phi_filtre[i] - d_phi_filtre[i + 10]) > 0.01 or abs(d_phi_filtre[i]) > 0.01: 
        i += 1
    i0 = i
    #valeur minimale de début de fluidisation
    d_S_fluidisation = max(d_phi_filtre[i0:])/2
    while d_phi_filtre[i] < d_S_fluidisation:
        i += 1
    
    plt.figure('non filtré')
    plt.plot(L_t[i0:],L_phi[i0:],'royalblue')
    plt.xlabel('temps (s)')
    plt.ylabel('porosité')
    plt.title('évolution de la porosité avec le temps (signal non filtré)')
    plt.plot()
    
    plt.figure('filtré')
    plt.plot(L_t[i0:],S[i0:],'orangered')
    plt.xlabel('temps (s)')
    plt.ylabel('porosité')
    plt.title('évolution de la porosité avec le temps (signal filtré)')
    plt.show()

    plt.figure('dérivée filtré')
    plt.plot(L_t[i0+1:],d_phi_filtre[i0:],'seagreen')
    plt.plot((L_t[i],L_t[i]),(2*max(d_phi_filtre[i0:]),2*min(d_phi_filtre[i0:])),"--", color = "#8099b2")
    plt.plot((0,T),(d_S_fluidisation,d_S_fluidisation),"--", color = "#8099b2")
    plt.xlabel('temps (s)')
    plt.ylabel('dérivée de la porosité par rapport au temps (s-1)')
    plt.title('variation de la porosité en fonction du temps (signal filtré)')
    plt.show()     
            
    return L_V[i]

determination_vitesse_fluidisation()
